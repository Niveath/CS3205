...
class Receiver:
    def __init__(self, sock, output_file, packet_size, window_size):
        ...
        self.window_buffer = [None] * self.max_seq_num
        ...
        try:
            while True:
                ...
                if seq_num == self.next_packet:
                    if self.window_buffer[seq_num] is None:
                        self.window_buffer[seq_num] = data

                        while self.window_buffer[self.next_packet] is not None:
                            self.file.write(self.window_buffer[self.next_packet])
                            self.window_buffer[self.next_packet] = None
                            self.next_packet = (self.next_packet + 1) % self.max_seq_num

                        # Send an cumulative ACK for the received packet
                    ...
                    if(random.random() >= self.drop_prob):
                        ...
                        if(self.debug):
                            print("Simulating ACK loss, ack number:", seq_num)
