...
class Sender:
    def __init__(self, sock, client_address, input_file, packet_size, window_size, rto):
        self.debug = False
        self.drop_prob = 0 # For simple testing
    ...
    def check_timeouts(self):
        while(self.min_unacked < self.num_packets):
            ...
            if(min_send_time != -1 and time.time() - min_send_time >= self.rto):
                ...
                packet = self.packet_handler.get_file_packet(self.min_unacked, self.min_unacked % self.max_seq_num)
                self.sock.sendto(packet, self.client_address)

                for i in range(self.min_unacked, self.next_packet):
                    self.send_times[i % self.max_seq_num] = time.time()

                if(self.debug):
                    print("Resending packet :", self.min_unacked)
            self.base_lock.release()
            self.time_lock.release()
...
def main():
    ...
    sender = Sender(sock, client_address, input_file, packet_size, window_size, rto)
