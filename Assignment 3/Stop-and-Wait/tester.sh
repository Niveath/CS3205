...
bandwidth=8
loss_values=("0.001" "0.005" "0.01" "0.015" "0.02" "0.05")  # Example loss values in percentage (0.1% to 1%)
...
for loss in "${loss_values[@]}"; do
    for delay in "${delay_values[@]}"; do
        ...
        # sudo tc qdisc change dev lo root netem delay $delay $deviation distribution normal
        ...
        python3 -u receiver.py "$output_file" "$packet_size" "$window_size" > receiver.txt 2> temp.txt &

        # Run sender.py in the background and redirect output to /dev/null
        # python3 sender.py "$input_file" "$packet_size" "$loss" "$rto" > sender.txt 2>&1 &
